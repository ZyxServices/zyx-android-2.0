...
public class EquipSearchFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener{
    View view;
    ...
    Handler handler=new Handler(){
        ...
        public void handleMessage(Message msg) {
            switch (msg.what){
                case HANDLE_EQUIP_DATA:
                    adapter=new SearchEquipAdapter(getActivity(),list);
    ...
    private void setNetData(){
        ArrayList<String> cacheData= (ArrayList<String>) CacheUtils.readJson(getActivity(), EquipSearchFragment.this.getClass().getName() + ".json");
        if (cacheData==null||cacheData.size()==0) {
            new Thread() {
                ...
                public void run() {
                    ...
                    JSONParseUtil.parseNetDataSearchEquip(getActivity(),result, EquipSearchFragment.this.getClass().getName()+".json",list, handler, HANDLE_EQUIP_DATA);
                }
            }.start();
        }else {
            JSONParseUtil.parseLocalDataSearchEquip(getActivity(), EquipSearchFragment.this.getClass().getName()+".json",list, handler, HANDLE_EQUIP_DATA);
        }
    }
}
