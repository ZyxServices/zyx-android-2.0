...
public class AllVenueFragment extends BaseFragment implements  SwipeRefreshLayout.OnRefreshListener{
    ...
    private void setData() {
        ...
        ArrayList<String> cacheData= (ArrayList<String>) CacheUtils.readJson(getActivity(), AllVenueFragment.this.getClass().getName() + ".json");
        if (cacheData==null||cacheData.size()==0) {
            new Thread() {
                ...
                public void run() {
                    ...
                    JSONParseUtil.parseNetDataVenue(getActivity(),result,AllVenueFragment.this.getClass().getName()+".json",datas, handler, HANDLE_DATA);
                }
            }.start();
        }else {
            JSONParseUtil.parseLocalDataVenue(getActivity(),AllVenueFragment.this.getClass().getName()+".json",datas, handler, HANDLE_DATA);
        }
    }
